...
import type { CategoryOutlook, CategorySummary, NewsArticle } from '@/types';
import {
  createApiError,
...
const buildCombinedPrompt = (categoryName: string, articles: NewsArticle[]): string => {
  const sentimentArrows: Record<string, string> = { positive: '↑', negative: '↓', neutral: '→' };
  const headlines = articles
    .slice(0, MAX_OUTLOOK_HEADLINES)
    .map((article) => `${sentimentArrows[article.sentiment] ?? '→'} ${article.title}`)
  ...
  return `Je bent een financieel analist. Analyseer onderstaande nieuwskoppen voor "${categoryName}".
...
{
  "summary": {
    "text": "3-4 beknopte zinnen samenvatting in het Nederlands. Focus op prijsbewegingen, risicosignalen en kansen voor beleggers.",
    "recommendation": "STRONG_SELL | SELL | HOLD | BUY | STRONG_BUY",
    "reasoning": "1 korte zin in het Nederlands die je beleggingsindicatie onderbouwt."
  },
  "outlook": {
    "dataPoints": [
      { "year": 0, "score": <huidig sentiment -10 tot 10> },
...
Voor summary.recommendation: kies EXACT één van STRONG_SELL, SELL, HOLD, BUY, STRONG_BUY.
Voor outlook scores: -10 = extreem negatief, 0 = neutraal, +10 = extreem positief.
Baseer je op: nieuwssentiment, analistenconsensus, sector-/markttrends, winstmarges en groeiprognoses.
Geef maximaal 3 bullish en 3 bearish factoren, in het Nederlands.`;
};
...
const parseCombinedResponse = (raw: string): CombinedAnalysis | null => {
  try {
    const jsonMatch = raw.match(/\{[\s\S]*\}/);
    if (!jsonMatch) return null;

    const parsed = JSON.parse(jsonMatch[0]) as { summary?: CategorySummary; outlook?: CategoryOutlook };

    const s = parsed.summary;
    if (!s || typeof s.text !== 'string' || !s.text || !VALID_RECOMMENDATIONS.has(s.recommendation)) return null;
    if (typeof s.reasoning !== 'string') s.reasoning = '';

    const o = parsed.outlook;
    if (!o || !Array.isArray(o.dataPoints) || o.dataPoints.length === 0) return null;
    if (!Array.isArray(o.bullish)) o.bullish = [];
    if (!Array.isArray(o.bearish)) o.bearish = [];
    if (typeof o.summary !== 'string') o.summary = '';

    for (const point of o.dataPoints) {
      point.score = Math.max(-10, Math.min(10, Number(point.score) || 0));
      point.year = Number(point.year) || 0;
    }

    return {
      summary: { text: s.text, recommendation: s.recommendation, reasoning: s.reasoning },
      outlook: { ...o, generatedAt: Date.now() },
    };
  } catch {
    return null;
  }
...
/** Successful combined analysis result. */
...
/** Failed combined analysis result. */
...
/**
 * Generates both a daily summary and a 10-year outlook in a single Gemini API call.
 * Each part is cached independently (summary per day, outlook for 30 days).
 */
export const generateCategoryAnalysis = async (
  ...
  try {
    response = await fetchWithRetry(`${GEMINI_ENDPOINT}?key=${apiKey}`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        contents: [{ parts: [{ text: buildCombinedPrompt(categoryName, articles) }] }],
      }),
    });
  } catch {
    return { summary: null, outlook: null, error: createApiError('Kan geen verbinding maken met de Gemini API.') };
  }
  ...
  if (!response.ok) {
    ...
    return {
      summary: null,
      outlook: null,
      error: createApiError(`Gemini API gaf status ${statusCode} terug.`, statusCode),
    };
  }
  ...
  const result = parseCombinedResponse(rawText);
  if (!result) {
    return {
      summary: null,
      outlook: null,
      error: createEmptyResponseError('De AI gaf geen bruikbaar antwoord terug.'),
    };
  }

  saveSummaryToCache(categoryId, result.summary);
  saveOutlookToCache(categoryId, result.outlook);

  return { summary: result.summary, outlook: result.outlook, error: null };
};
